...
/**
 * Entity for a Cache Container
 * 
 * @author Juergen Edelbluth <dev@jued.de>
 * @since 0.1
 * @version 0.1
 */
@Entity @Table(name = "CONTAINER")
@NamedQueries({
    @NamedQuery(name = "Container.findByUUID", query = "select c from Container c where c.uuid=:uuid")
})
public class Container implements Serializable
{
    
    @Id @Column(name = "SYS_UUID", insertable = true, nullable = false, updatable = false, length = 16) private byte[] uuid;
    @Column(name = "CONTAINER_NAME", insertable = true, nullable = false, updatable = true, unique = true, length = 255) private String name;
