...
CREATE TABLE CACHEFLAGS
(
    USER_FLAG CHAR(16) FOR BIT DATA NOT NULL REFERENCES CACHE_USER_FLAG (SYS_UUID) ON DELETE CASCADE,
    GEOCACHE CHAR(16) FOR BIT DATA NOT NULL REFERENCES GEOCACHE (SYS_UUID) ON DELETE CASCADE,
    PRIMARY KEY(USER_FLAG, GEOCACHE)
);
...
(
    CACHE CHAR(16) FOR BIT DATA NOT NULL REFERENCES CACHEDATA (SYS_UUID) ON DELETE CASCADE,
    TRAVELBUG CHAR(16) FOR BIT DATA NOT NULL REFERENCES TRAVELBUG (SYS_UUID) ON DELETE RESTRICT,
    PRIMARY KEY(CACHE, TRAVELBUG)
);

CREATE TABLE WPTSSYMBOL
(
    SYS_UUID CHAR(16) FOR BIT DATA,
    SYMBOL_NAME VARCHAR(255) NOT NULL,
    IMG_DATA BLOB(255K),
...
CREATE TABLE ADDITIONALWPTS
(
    SYS_UUID CHAR(16) FOR BIT DATA,
    CACHE CHAR(16) FOR BIT DATA NOT NULL REFERENCES CACHEDATA (SYS_UUID) ON DELETE CASCADE,
    GC_CODE VARCHAR(255) NOT NULL,
    WP_NAME VARCHAR(1023),
    COMMENT CLOB(255K),
    LONGITUDE DOUBLE PRECISION,
    LATITUDE DOUBLE PRECISION,
    SYMBOL CHAR(16) FOR BIT DATA DEFAULT NULL REFERENCES WPTSSYMBOL (SYS_UUID) ON DELETE RESTRICT,
    CREATED TIMESTAMP,
    PRIMARY KEY(SYS_UUID)
);
...
(
    SYS_UUID CHAR(16) FOR BIT DATA,
    CACHE CHAR(16) FOR BIT DATA NOT NULL REFERENCES GEOCACHE (SYS_UUID) ON DELETE CASCADE,
    CREATED TIMESTAMP,
